 ...
 */
	...
	$conn=db_connect();
	$user_info="select user.username,user.user_img,article.art_id,article.art_content,article.cur_time,article.likes from user,article where user.username=article.username and user.username='".$username."' order by article.art_id desc limit $start,5";
